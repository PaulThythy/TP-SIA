...
void initBuffers() {
  //surface de Bézier
  GLfloat bezierSurfaceControlPoints[] = {
    //p00           //p10           //p20
  ...
  GLfloat bezierSurfaceColorPoints[] = {
    //p00           //p10           //p20
  ...
  // Création du VBO pour indices verticals
  GLuint bezierVerticalIndices[] = {
    0, 3, 6,    // première colonne
    1, 4, 7,    // deuxième colonne
    2, 5, 8     // troisième colonne
  };

  glGenBuffers(1, &vbo_bezier_indices);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_bezier_indices);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(bezierVerticalIndices), bezierVerticalIndices, GL_STATIC_DRAW);

  if (glIsBuffer(vbo_bezier_surf_position) == GL_TRUE)
      glDeleteBuffers(1, &vbo_bezier_surf_position);
  glGenBuffers(1, &vbo_bezier_surf_position);
  glBindBuffer(GL_ARRAY_BUFFER, vbo_bezier_surf_position);
  glBufferData(GL_ARRAY_BUFFER, sizeof(bezierSurfaceControlPoints), bezierSurfaceControlPoints, GL_STATIC_DRAW);

  if (glIsBuffer(vbo_bezier_surf_colors) == GL_TRUE)
      glDeleteBuffers(1, &vbo_bezier_surf_colors);
  glGenBuffers(1, &vbo_bezier_surf_colors);
  glBindBuffer(GL_ARRAY_BUFFER, vbo_bezier_surf_colors);
  glBufferData(GL_ARRAY_BUFFER, sizeof(bezierSurfaceColorPoints), bezierSurfaceColorPoints, GL_STATIC_DRAW);
  ...
  //cercle
  float step = 2*M_PI/circleResolution;
  float radius = 1.0f; 
  glm::vec3 origin(0.5f,0.0f,0.5f);
  GLfloat circlePoints[circleResolution * 3];
  GLfloat circleColorPoints[circleResolution * 3];

  for(int i = 0; i < circleResolution; i++) {
    circlePoints[i] = radius * cos(i * step) + origin.x;
    circlePoints[i*1] = 0.0f;
    circlePoints[i*2] = radius * sin(i * step) + origin.z;

    circleColorPoints[i] = 0.0f;
    circleColorPoints[i*1] = 1.0f;
    circleColorPoints[i*2] = 0.0f;
  }

  if (glIsBuffer(vbo_circle_position) == GL_TRUE)
  ...
  if (glIsBuffer(vbo_circle_colors) == GL_TRUE)
      glDeleteBuffers(1, &vbo_circle_colors);
  glGenBuffers(1, &vbo_circle_colors);
  glBindBuffer(GL_ARRAY_BUFFER, vbo_circle_colors);
  glBufferData(GL_ARRAY_BUFFER, sizeof(circleColorPoints), circleColorPoints, GL_STATIC_DRAW);
...
//-------------------------------------
{
  ...
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_bezier_indices);
  // Première colonne
  glDrawElements(GL_LINE_STRIP, 3, GL_UNSIGNED_INT, (void*)(0 * sizeof(GLuint)));
  // Deuxième colonne
  glDrawElements(GL_LINE_STRIP, 3, GL_UNSIGNED_INT, (void*)(3 * sizeof(GLuint)));
  // Troisième colonne
  glDrawElements(GL_LINE_STRIP, 3, GL_UNSIGNED_INT, (void*)(6 * sizeof(GLuint)));
